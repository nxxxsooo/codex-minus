...
主窗口完成首屏加载后，每个进程只启动一次非阻塞检查。检查期间不覆盖当前通知、不阻塞供应商、会话或目录操作。
...
顶部工具栏新增更新按钮。状态为：空闲、检查中、可更新、下载中、安装中、失败。可更新时显示轻量圆点；手动检查没有新版时明确提示「当前已是最新版本」。
...
- Windows x64／arm64：NSIS `.exe` 与对应 `.sig` 作为自动更新通道；MSI 与 NSIS 继续作为手工安装附件。
...
2. 从该版本开始，后续正式版本可在应用内更新。
3. macOS Gatekeeper／Windows SmartScreen 仍可能因平台代码签名状态给出提示；updater 签名只验证更新来源和完整性。
...
- tag 与六处版本源一致性测试。
- `latest.json` schema、三平台完整性、HTTPS URL、内联签名和附件存在性测试。

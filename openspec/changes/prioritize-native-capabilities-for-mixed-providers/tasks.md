...
- [ ] 10.6 Restore coverage for `StagingRejected`, whose only exercised trigger was removed by 10.3. No trigger is reachable through the public commit API: invalid common config is sanitized before staging, and the observation hook exposes no staging checkpoint. Add a staging checkpoint to the hidden observer, matching how catalog materialization is already injectable, before asserting the code.

...
- [ ] 1.4 Add failing frontend/backend contract tests for the provider-owned commit envelope: canonical topology draft, catalog drafts, action, focused and previous active IDs, Context confirmation, draft revision, and expected provider fingerprint for new/existing detail saves plus enable, reorder, copy, delete, aggregate-cleanup, and test-model mutations.
- [ ] 1.5 Implement thin detail/topology request adapters over one compare-and-swap schema and planner; make ordinary nested catalog controls draft-only, echo revision without treating it as concurrency control, and prevent `model_catalog_status` from creating a new profile's missing catalog state as a post-save side effect while preserving versioned catalog migrations.

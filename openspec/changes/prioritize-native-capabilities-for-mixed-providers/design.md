...
Introduce a provider-owned commit envelope rather than accepting an arbitrary whole-application settings overwrite. Its canonical projection contains provider enablement, the ordered provider and aggregate lists, active IDs, legacy provider projections still required by the pinned core, and provider test-model state. The request also contains zero or more profile catalog drafts, an optional focused profile ID, the intended action (`save` or `setCurrent`), the previous active profile ID, Context-cleanup confirmation, the frontend draft revision, and an expected provider-state fingerprint. For a new profile, the page constructs an in-memory implicit `official-plus-custom` catalog draft before first Save. `model_catalog_status` cannot create a new profile's missing catalog state as a post-save side effect; existing versioned state migrations remain governed by the catalog capability.

Provider-detail and provider-list/topology operations may use separate thin Tauri commands for ergonomic payloads, but both are adapters into this one planner and transaction engine. The backend reloads persisted settings, compare-and-swaps the expected provider fingerprint, replaces only the provider-owned canonical projection, and preserves all unrelated application settings. `draftRevision` is echoed so the frontend can discard out-of-order responses; it is not a concurrency guard and never substitutes for the backend fingerprint.

This envelope also owns provider enable/disable, reorder, copy, delete, aggregate-reference cleanup, and provider test-model mutations. Copy/new operations supply a deterministic catalog draft; deletion removes or reconciles the deleted profile's catalog state and aggregate references in the same plan. Therefore hardening generic `save_settings` does not strand existing list controls or create a second unprotected mutation path.
...
The generic `save_settings` Tauri command is also hardened at the backend, not merely avoided by the detail page. Before that hardening lands, every provider-detail and provider-list caller—including enablement, reorder, copy, delete, aggregate cleanup, and provider test-model changes—is routed through a thin adapter to the provider-owned engine. Generic `save_settings` then compares the incoming provider-owned settings snapshot—including relay profiles, active IDs, common/context relay configuration, provider enablement, aggregate/provider legacy projections, and related relay fields—with the persisted snapshot. A request may save unrelated application settings only when that provider-owned snapshot is unchanged; any provider-bearing difference is rejected and directed to the provider-owned engine. This prevents stale or custom frontend callers from bypassing provider TOML validation and atomic catalog/live planning.

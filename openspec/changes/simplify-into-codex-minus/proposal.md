...
- **Catalog UI reduction**: the 官方模型目录 band, the mode segmented control, the official override table (the "many boxes"), upstream-topology and external-adopt controls disappear from the default flow; the custom model list remains as 配置模型. Existing external-catalog profiles keep working untouched.

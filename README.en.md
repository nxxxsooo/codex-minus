...
Codex Minus is a trimmed fork of [Codex++ Manager](https://github.com/BigPizzaV3/CodexPlusPlus). It keeps provider switching, model catalogs, local session lifecycle, and configuration diagnostics while removing renderer injection, launchers, marketplaces, and auto-update.
...
After verification, unzip the archive and move `Codex Minus.app` to `/Applications`. The current build uses an ad-hoc signature and is not signed with Developer ID or notarized by Apple. If macOS blocks the first launch, use Open Anyway in System Settings, Privacy & Security.
...
Quit Codex Minus, download the latest release, and replace `/Applications/Codex Minus.app`. The app has no auto-updater, and a GitHub Release does not update an installed copy automatically.
...
- Chat Completions and aggregator profiles depend on the upstream launcher's proxy at `127.0.0.1:57321`. Codex Minus does not ship that proxy, so these profiles should not be used.

...
describe("one model table answers every model question", () => {
  ...
  it("lists the models Codex lists, and lets any of them be deleted", () => {
    ...
    assert.match(catalogEditor, /const shownOfficial = catalogOfficialRows\(overlay, officialModels, mode\)/);
    assert.match(catalogEditor, /const shownCustom = catalogCustomRows\(overlay, officialModels, mode\)/);
  ...
  it("can put a deleted model back and repair the whole list at once", () => {
    ...
    assert.match(restore, /catalogRestoreLosses\(\{ overlay, officialModels, wanted: proSlugs, mode \}\)/);
    assert.match(restore, /await confirmDesktop\(/, "a restore that removes rows says which ones");

...
- **Legacy model-list reset**: Startup may reset only implicit `official-plus-custom` state through the coordinator: remove exact `legacy-model-list` rows and official overrides exactly equal to legacy reconstruction; repair a removed default to `gpt-5.6-terra` only when it survives in neither the visible official baseline nor retained custom rows; preserve every other mode, explicit/external state, preset, user-created or unknown-provenance row, and modified/user-added override. Explicit overlay edits establish ownership. This narrow migration is not permission to run the core normalizer or rewrite any other profile contract.
